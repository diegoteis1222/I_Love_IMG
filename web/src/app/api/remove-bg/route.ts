...
/**
 * Proxy hacia el microservicio Python (rembg). El front nunca habla
 * directamente con el servicio de IA — siempre pasa por aquí.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    const res = await fetch(`${AI_SERVICE_URL}/remove-background`, {
      method: 'POST',
      body: upstream
    });
    ...
    if (!res.ok) {
      ...
      console.error('[/api/remove-bg] AI service error:', res.status, text);
      return NextResponse.json(
    ...
    const description = describeRemoveBg();
